...
class Client:
    ...
    async def get_domains(self) -> List[DomainModel]:
        async with self._session.get("/api/v3/domains") as response:
            ...
            return [
                DomainModel(domain['name'], domain['type'], domain['forward_available'], domain['forward_max_seconds'])
    ...
    async def get_messages(self,
                           email: str) -> Optional[List[MessageResponseModel]]:
        async with self._session.get(f"/api/v3/email/{email}/messages") as response:
            response_json = await response.json()
            if len(response_json) == 0:
                return None
            return [
                MessageResponseModel(id=message['id'], subject=message['subject'], cc=message['cc'],
                                     body_html=message['body_html'], body_text=message['body_text'],
                                     attachments=message['attachments'], created_at=message['created_at'],
                                     email_from=message['from'],
                                     email_to=message['to']) for message in response_json]
